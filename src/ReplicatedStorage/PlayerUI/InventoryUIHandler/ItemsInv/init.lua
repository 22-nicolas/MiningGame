...
function ItemsInv:update(inventoryData)
	if not inventoryData and not self.inventoryData then return end
	...
	if #self.Slots ~= self.playerUI.stats.invSlots then
		warn("[InventoryUIHandler] Timed out while waiting for inv to be initiated. Player: "..tostring(self.player.UserId))
		return
	end
